## AWS Load CSV In DynamoDB
This is a sample that shows how you can build a Python AWS Lambda service that is triggered from an S3 bucket CSV, and then upload CSV content in Database.

The following diagram illustrates the design:
![Architecture Diagram](figures/CSV-loader-diagram.png)

## How to build and run this sample in your AWS Cloud Instance
### You need to do the following changes in `loadCSVInDynamo.py`:
1. Change `region` to the name of your DynamoDB table's region:
```
region = 'us-east-2'
```

2. Change `tableName` to the name of your DynamoDB table:
```
tableName = 'Country'
```

3. Change `sourceBucketName` to the name of the bucket from which you will upload the CSV file:
```
sourceBucketName = 'country-csv-loader-bucket'
```

4. Create a bucket with the name that you specify in `sourceBucketName`.
